(() => {
  ...
  const MERCHANT_APP_BUILD = '2026.09.13.04';
  ...
  async function login() {
    document.body.innerHTML = `<main class="ap-login-shell"><section class="ap-login-card" data-login-panel="merchant"><div class="ap-login-brandline"><span class="ap-login-mark">AS</span><div><strong>AP Service</strong><small class="merchant-build-badge" data-merchant-build style="display:inline-block;margin-left:6px;padding:1px 7px;border-radius:99px;background:rgba(11,140,124,.14);font-size:10px;line-height:1.6;vertical-align:middle;white-space:nowrap">v${MERCHANT_APP_BUILD}</small><small>ศูนย์จัดการร้านค้า เมนู และออร์เดอร์</small></div></div><span class="ap-login-role">Merchant Login</span><h1 class="ap-login-title">เข้าสู่ระบบ Merchant</h1><p class="ap-login-intro">ใช้บัญชีที่ Admin สร้างและผูกสิทธิ์กับร้านค้าที่ดูแลแล้ว</p><form id="login" class="ap-login-form"><label class="ap-login-field"><span>ชื่อผู้ใช้ Merchant</span><div class="ap-login-control"><span class="ap-login-icon" aria-hidden="true">${window.APLoginUI?.icon('user') || window.APLoginUI?.icon('mail') || ''}</span><input id="username" type="text" autocomplete="username" autocapitalize="none" spellcheck="false" aria-label="ชื่อผู้ใช้ Merchant" placeholder="เช่น store01" required></div></label><label class="ap-login-field"><span>รหัสผ่าน</span><div class="ap-login-control"><span class="ap-login-icon" aria-hidden="true">${window.APLoginUI?.icon('lock') || ''}</span><input id="password" type="password" autocomplete="current-password" aria-label="รหัสผ่าน" placeholder="กรอกรหัสผ่านของคุณ" required><button class="ap-login-password-toggle" type="button" data-password-toggle aria-controls="password" aria-label="แสดงรหัสผ่าน">${window.APLoginUI?.icon('eye') || ''}</button></div></label><button class="ap-login-submit" data-login-submit type="submit">เข้าสู่ระบบ</button><p class="ap-login-status" data-login-status aria-live="polite"></p></form><div class="ap-login-admin-note"><span aria-hidden="true">${window.APLoginUI?.icon('shield') || ''}</span><div><strong>บัญชีนี้สร้างโดย Admin เท่านั้น</strong><p>หากยังไม่มีบัญชี โปรดให้ผู้ดูแลระบบสร้างและผูกสิทธิ์ก่อนเข้าสู่ระบบ</p></div></div></section></main>`;
