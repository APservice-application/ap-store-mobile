(() => {
  ...
  const MERCHANT_APP_BUILD = '2026.09.13.05';
  ...
  async function store() {
    ...
    $('#save').onsubmit = async event => {
      event.preventDefault();
      try {
        await invokeEdge({ action: 'merchant_update_store', data: { name: $('#name').value.trim(), description: $('#description').value.trim(), phone: $('#phone').value.trim(), eta: $('#eta').value.trim(), payout_method: $('#payoutMethod').value, payout_bank_name: $('#payoutBank').value.trim(), payout_account_name: $('#payoutName').value.trim(), payout_account_number: $('#payoutNumber').value.trim(), ...draftMedia } });
        $('#storeSaveStatus').textContent = 'บันทึกข้อมูลร้านแล้ว'; M.ui.setNotice('บันทึกข้อมูลร้านแล้ว');
